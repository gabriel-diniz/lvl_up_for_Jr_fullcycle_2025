...
export class PartnerModel {
    ...
    static async findById(id: number, options?: {user?: boolean }): Promise<PartnerModel | null>{
        ...
        if (options?.user){
            query = "SELECT p.*, users.id as user_id, users.name as user_name, users.email as user_email FROM partners p JOIN users ON p.id = users.id WHERE p.id = ?";
        }
    ...
    static async findByUserId(userId: number, options?: {user?: boolean }): Promise<PartnerModel | null>{
        const db = DataBase.getInstance();
